...
def decipherIType(trailer):
    result = format(int(trailer[0][1:]), "05b") + format(int(trailer[1][1:]), "05b") + format(int(trailer[2]), "016b")
    return result
...
def simulator(curDir):
    with open(os.path.join(curDir , 'imem.txt'), 'w') as outputFile, open(os.path.join(curDir , 'Instructions.txt'), 'r') as inputFile:
        print(curDir)
...
if __name__ == '__main__':
    ...
    srcDmemFile = os.path.join(curDir, "dmem.txt")
